...
int main(int argc, char *argv[])
{
   ...
   g = 0.0;
   g[dim - 1] = 1;
   VectorConstantCoefficient load(g);
   b.AddDomainIntegrator(new VectorDomainLFIntegrator(load));
   b.Assemble();
   ...
   Vector rhov(dim);
   rhov = 1.0;
   VectorConstantCoefficient rho(rhov);
   a.AddDomainIntegrator(new VectorMassIntegrator(rho));
